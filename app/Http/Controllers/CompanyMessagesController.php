...
class CompanyMessagesController extends Controller
{
    ...
    public function fileupload_action(Request $request) {
    {
         ...
         if($validation->passes())
         {
              ...
              $image->move(public_path('images/chats'), $new_name);
               ...
               $pusher = new Pusher(
                   env('PUSHER_APP_KEY','a28b149bfa9cd891c76e'),
                   env('PUSHER_APP_SECRET','f9bd83f202cd75455a0a'),
                   env('PUSHER_APP_ID','962744'),
                   $options
                );
    ...
    public function post_user_messages(Request $request) {
        ...
        $job_id = $request->job_id;
        $from =  Auth::guard('company')->user()->id;
        $fromName = Auth::guard('company')->user()->name;
        ...
        /* new code */
         $content = $fromName." has sent you a New message";
        ...
        $notification = Notification::create($newMsgNotification);
        ...
        $pusher->trigger('company-new-message', 'company-new-message-event', $newMsgNotification);
